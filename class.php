					...
					<div class="card-footer">
						<div class="row">
							<div class="col-md-12">
								...
								<button class="btn btn-sm btn-default col-sm-3" type="reset"> Annuler</button>
							</div>
						</div>
					</div>
				</div>
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_class" type="button" data-id="<?php echo $row['id'] ?>"  data-course_id="<?php echo $row['course_id'] ?>"  data-level="<?php echo $row['level'] ?>" data-section="<?php echo $row['section'] ?>" >Modifier</button>
										<button class="btn btn-sm btn-danger delete_class" type="button" data-id="<?php echo $row['id'] ?>">Supprimer</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('#manage-class').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Donnée sauvegarder avec succes",'success')
					setTimeout(function(){
						location.reload()
					},1500)

				}else if(resp == 2){
				$('#msg').html('<div class="alert alert-danger mx-2">Classe deja presente.</div>')
				end_load()
				}				
			}
		})
	...
	$('.delete_class').click(function(){
		_conf("Etes-vous sure de vouloir supprimer cette classe?","delete_class",[$(this).attr('data-id')])
	})
	function delete_class($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Donnée supprimer avec succes",'success')
					setTimeout(function(){
						location.reload()
					},1500)
